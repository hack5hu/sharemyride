...
export const mapBackendRideToUI = (
  ride: any,
  type: 'upcoming' | 'completed' | 'requests' | 'archive',
  t: any,
) => {
  ...
  try {
    ...
    dayMonth = startTime.toLocaleDateString('en-US', {
      day: 'numeric',
      month: 'short',
    });
  } catch (e) {}
  ...
  if (type === 'upcoming') {
    if (isDriver) {
      statusTag =
        ride.status === 'SCHEDULED' || ride.status === 'CONFIRMED'
          ? t('myRides.confirmedStatus')
          : '';
    } else {
      statusTag =
        ride.status === 'PENDING'
          ? t('myRides.pendingStatus')
          : ride.status === 'SCHEDULED' || ride.status === 'CONFIRMED'
          ? t('myRides.confirmedStatus')
          : '';
    }
    ...
    if (diffMins < 0) {
      ...
      timerLabel = `${t('myRides.startsIn')} ${diffHours}${t(
        'myRides.hours',
      )} ${diffMins % 60}${t('myRides.mins')}`;
    } else {
      timerLabel = `${weekday}, ${dayMonth}`;
    }
  } else if (type === 'requests') {
    timerLabel = `Requested ${timeStr}`;
    statusTag = `${ride.seatCount || 1} Seat${
      (ride.seatCount || 1) > 1 ? 's' : ''
    }`;
  } else if (type === 'archive') {
    statusTag =
      ride.status === 'CANCELLED'
        ? t('common.cancelled')
        : t('myRides.completedStatus');
  ...
  return {
    ...
    carModel: isRequest
      ...
      ? `${ride.vehicleType?.replace('_', ' ') || 'CAR'} (${
          ride.vehicleRegistration
        })`
      : isDriver
