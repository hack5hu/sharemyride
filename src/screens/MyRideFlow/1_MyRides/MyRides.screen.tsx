...
export const MyRidesScreen: React.FC = memo(() => {
  ...
  const renderItem = useCallback(
    ({ item, activeTab: tab }: { item: any; activeTab: string }) => (
      <RideItem
        item={item}
    ...
    ({ item: tab }: { item: (typeof TABS)[number] }) => {
      ...
      const emptyComponent = (
        <EmptyState
          ...
          title={t(
            `myRides.no${tab.charAt(0).toUpperCase() + tab.slice(1)}Title`,
          )}
          description={t(
            `myRides.no${tab.charAt(0).toUpperCase() + tab.slice(1)}Desc`,
          )}
        />
      );
      ...
      const footerComponent =
        isLoading && !isRefreshing && tab === activeTab ? (
