...
export const BubbleWrapper = styled.View<{ isSender: boolean }>`
  ...
  border-bottom-left-radius: ${({ isSender }) =>
    isSender ? moderateScale(20) : 0}px;
  border-bottom-right-radius: ${({ isSender }) =>
    isSender ? 0 : moderateScale(20)}px;
  min-width: ${scale(60)}px;
`;

export const TouchableBubbleWrapper = styled.TouchableOpacity<{
  isSender: boolean;
}>`
  background-color: ${({ theme, isSender }) =>
    isSender
