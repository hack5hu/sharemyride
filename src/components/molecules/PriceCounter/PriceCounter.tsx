...
const Card = styled.View<{ variant: 'default' | 'compact' }>`
  ...
  elevation: 2;
  gap: ${({ variant }) =>
    variant === 'compact' ? verticalScale(8) : verticalScale(20)}px;
`;
...
const PriceLabel = styled.Text<{ variant: 'default' | 'compact' }>`
  ...
  font-weight: 500;
  font-size: ${({ variant }) =>
    variant === 'compact' ? responsiveFont(10) : responsiveFont(12)}px;
...
const PriceText = styled.Text<{ variant: 'default' | 'compact' }>`
  ...
  font-weight: 800;
  font-size: ${({ variant }) =>
    variant === 'compact' ? responsiveFont(28) : responsiveFont(52)}px;
  color: ${({ theme }) => theme.colors.on_surface};
  letter-spacing: -1px;
  min-width: ${({ variant }) =>
    variant === 'compact' ? scale(70) : scale(110)}px;
  text-align: center;
`;
